...
export default function ContactForm() {
  ...
  const onSubmit = async (data: FormData) => {
    // Simulate network delay (Phase 1 — no backend)
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-cream rounded-2xl border border-navy/8 p-8 md:p-10">
      ...
      <div className="mb-5">
        ...
        <select
          className={`w-full px-4 py-3 rounded-lg border bg-white text-navy text-sm focus:outline-none focus:ring-2 focus:ring-gold/30 transition-shadow appearance-none ${
            errors.inquiryType ? 'border-red-400' : 'border-navy/15'
          }`}
          defaultValue=""
          {...register('inquiryType', { required: t('errors.inquiryTypeRequired') })}
        >
          <option value="" disabled>{t('inquiryTypePlaceholder')}</option>
          <option value="client">{t('client')}</option>
          <option value="candidate">{t('candidate')}</option>
        </select>
        {errors.inquiryType && (
          <p className="text-red-500 text-xs mt-1.5">{errors.inquiryType.message}</p>
        )}
      </div>
